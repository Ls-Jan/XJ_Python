...
class XJQ_PageNavigation(QWidget):#【半成品】页导航栏
	...
	def __init__(self,*args,iconLeft:str=GetRealPath('./V左箭头.png'),iconRight:str=GetRealPath('./V右箭头.png')):
		super().__init__(*args)
		self.__count=100
	...
	def setStyleSheet(self,qss:str,name:str='Main'):
		'''
			name可选值为CurrentPage、PageCount、ArrowLeft、ArrowRight，
			qss可为文件路径。
		...
		if(name=='CurrentPage'):
			self.__cPage.setStyleSheet(qss)
		elif(name=='PageCount'):
			self.__pCount.setStyleSheet(qss)
		elif(name=='ArrowLeft'):
			self.__aBtns['L'].setStyleSheet(qss)
		elif(name=='ArrowRight'):
			self.__aBtns['R'].setStyleSheet(qss)
		elif(name==None):
