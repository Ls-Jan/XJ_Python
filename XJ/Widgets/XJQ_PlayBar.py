...
class XJQ_PlayBar(QFrame):#播放条(以帧为单位)
	'''
		播放条，不单独使用
	'''
	...
	def Set_Index(self,index=None,total=None):#索引发生变化时返回真
		slider=self.__widSlider
		if(total==None):
			total=slider.maximum()
		if(index==None):
			index=slider.value()
		v0=slider.value()
		...
		v2=slider.value()
		self.__widIndex.setText(self.indexFormat(v2,total))
