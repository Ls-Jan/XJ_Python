...
from PyQt5.QtGui import QPainter,QBitmap,QColor,QCursor
...
class XJQ_PureColorIconButton(QAbstractButton):
	...
	'''
	def __init__(self,icon:Union[XJQ_PureColorIcon,str]=None,parent:QWidget=None):
		'''
			icon将直接传给XJQ_PureColorIcon
		'''
		super().__init__(parent)
		self.__rRadius=20
		self.__rMask=False
		self.__cMask=False
		self.__icon=XJQ_PureColorIcon(size=(32,32))
	...
	def Set_Icon(self,icon:Union[XJQ_PureColorIcon,str]):
		'''
			设置纯色图标。
			等同于调用setIcon
		'''
	...
	def Set_BgColor(self,default:QColor=None,hover:QColor=None,pressed:QColor=None,disable:QColor=None):
		...
		for key in self.__cache:
			...
			if(val):
				...
				keys.append(key)
		self.__UpdateCache(*keys)
	def Set_Mask(self,*,roundRect:bool=False,circle:bool=False,roundRectRadius:bool=None):
	...
	def setIcon(self,icon:Union[XJQ_PureColorIcon,str]):
		'''
			设置纯色图标
		'''
		icon=XJQ_PureColorIcon(icon,size=self.__icon.size())
		self.__icon=icon
		self.__UpdateCache()
