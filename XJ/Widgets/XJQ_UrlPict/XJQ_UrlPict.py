...
class XJQ_UrlPict(XJQ_AutoSizeLabel):
	...
	def __init__(self,config:UrlPictConfig,url:QUrl,data:QByteArray=None,timeout:float=0,func_shrinkedSize=None):
		'''
	...
	def Get_Bytes(self):
		'''
			获取图片文件数据，返回bytes或是None
		'''
		return self.__config.cacheProxy.Get_UrlData(self.__url)
